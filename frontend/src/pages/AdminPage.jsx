...
export default function AdminPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      const token = await getToken();
      if (target.type === 'usuario') await adminGateway.removerUsuarioPermanente(token, target.item.id);
      if (target.type === 'peca') await adminGateway.removerPecaPermanente(token, target.item.id);
  ...
  if (dash.state.loading) return <div className="admin-page"><Header /><div className="admin-full-state"><div className="admin-spinner" /><p>{tr('Preparando seu painel…')}</p></div></div>;
  if (dash.state.error) return <div className="admin-page"><Header /><div className="admin-full-state"><h1>{tr('Acesso não autorizado')}</h1><p>{dash.state.error}</p><button onClick={() => navigate('/')}>{tr('Voltar ao início')}</button></div></div>;
  ...
  function dashboardWidget(key, index) {
    ...
    if (key === 'estoque_baixo') return <article className="admin-panel admin-stock-card admin-grid-regular" key={key}><div className="admin-panel-title"><div><h3>{tr('Atenção ao estoque')}</h3><p>{tr('Peças com três unidades ou menos.')}</p></div><span className={`admin-data-badge ${lowStock.length ? 'is-warning' : ''}`}>{lowStock.length} {tr('alertas')}</span></div>{lowStock.length ? <div className="admin-stock-list">{lowStock.map((peca) => <button key={peca.id} onClick={() => setSection('pecas')}><span><strong>{peca.nome_peca || `Peça #${peca.id}`}</strong><small>{peca.sku || tr('Sem SKU')}</small></span><b>{peca.estoque_atual ?? peca.estoque} {tr('un.')}</b></button>)}</div> : <div className="admin-healthy">✓ {tr('Estoque em níveis saudáveis')}</div>}</article>;
    ...
    if (key === 'seguranca') return <article className="admin-panel admin-security admin-grid-regular" key={key}><span className="admin-security-icon">✓</span><h3>{tr('Acesso protegido')}</h3><p>{tr('Você está autenticado com permissão administrativa. Todas as alterações passam pela API protegida.')}</p><div><span>{tr('Perfil')}</span><strong>{tr('Administrador')}</strong></div></article>;
    ...
    if (key === 'produtos_top') return <article className="admin-panel admin-top-products admin-grid-regular" key={key}><div className="admin-panel-title"><h3>{tr('Peças mais vendidas')}</h3><p>{tr('Ranking por unidades no período.')}</p></div>{productRanking.length ? productRanking.map((product, position) => <div className="admin-top-product" key={product.id}><b>{position + 1}</b><span><strong>{product.name}</strong><small>{money(product.revenue)}</small></span><em>{product.units} {tr('un.')}</em></div>) : <div className="admin-healthy">{tr('Sem vendas no período')}</div>}</article>;
    return null;
  }
